...
%   > D_e for the contraction segment is calculated in mNPS_readData.m based on
...
[out_j12, ~,~,~,~] = mNPS_procData(filepath_JOVE12, ch_height, De_np, wC, thresholds, fs, ASLS_param);

% % if you want to get the preprocessed data as well
% [out_j12, filtered_data_j12, y_baseline_j12, t_filtered_j12, fs_filtered_j12] = ...
%     mNPS_procData(filepath_JOVE12, ch_height, De_np, wC, thresholds, fs, ASLS_param);
...
% 
% [out_j10c, ~,~,~,~] = mNPS_procData(filepath_JOVE10c, ch_height, De_np, wC, thresholds, fs, ASLS_param, eventlength_filt);
